...
#   }
        ...
        mydivs = soup.find("div", {"class": "catalogue-info"})
        catalogue_uid=soup.select_one('#catalogue-detail')
        catalogue_uid=catalogue_uid["catalogue_uid"]
        li=mydivs.find_all('span')
        
        x12=li[0].getText().strip().replace("\n","")
        ...
        #print(year)
        try:
            x22=x22.replace(year,month,day)
        except:
        ...
        # only_month= x22.date()
        # print(only_month)
        # x22 = x22.strftime("/%m %d/%Y")
        x22=x22.strftime("%d-%b-%y")
        
        #print(x12)
        AuctionInfo["auction_start_date"]=x22
        h1detail=soup.select_one(".catalogue-detail-title")
        #print(h1detail.getText())
        #print(AuctionInfo)
        title=h1detail.getText()
        title=title.replace("Day Sale -","")
        auctionno=title.split("-")[0].replace("Auction","")
        auchouse="Lempertz"
        auctname=title.split("-")[1]
        AuctionInfo["auction_num"]=auctionno.strip()
        AuctionInfo["auction_house_name"]=auchouse.strip()
        AuctionInfo["auction_name"]=auctname.strip()
        AuctionInfo["auction_location"]=li[4].getText().strip()

        #print(AuctionInfo)
        #print(catalogue_uid)
        body={"catalogue_uid":catalogue_uid,"catalogue_type":"cataloguetList"}
        body= json.dumps(body) 
#         headers={
#             "Host": "www.lempertz.com",
...
#         }
        eq=requests.post("https://www.lempertz.com/en/?type=133799",data=body)
        #print(eq.json())
        #print(type(eq.json()))
        return eq.json(),AuctionInfo
...
def updatestatus(auctionno, auctionurl):
    ...
    pageurl = "http://216.137.189.57:8080/scrapers/finish/?scrapername=Lempertz&auctionnumber=%s&auctionurl=%s&scrapertype=2"%(auctionno, urllib.parse.quote(auctionurl))
    pageResponse = None
    try:
...
if __name__ == "__main__":
    ...
    pagesneeded = 10
    dictobj,MainInfo=getitemurls(auctionurl,int(pagesneeded))
    getitems(MainInfo,csvpath,dictobj)
    #getdetailfromurl(urllist[0])
...
#sample command>        
#https://www.lempertz.com/en/catalogues/detail/1188-1-day-sale-contemporary-art.html
