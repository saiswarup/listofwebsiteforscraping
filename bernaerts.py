...
class BernaertsBot(object):
    ...
    def __init__(self, auctionurl, auctionnumber):
        ...
        if cnps:
            ...
            sys.exit()
        #https://www.bernaerts.eu/tags/veiling_ajax.php?cookie=&cataloog=%s&zitting=3&sql_sort_method=0&start_page=0&show_rows=150&sql_lot_range=&lijst=0&search_mode=&search_data=&verkocht=1
        self.requestUrl = "https://www.bernaerts.eu/tags/veiling_ajax.php?cookie=&cataloog=%s&zitting=3&sql_sort_method=0&start_page=0&show_rows=250&sql_lot_range=&lijst=0&search_mode=&search_data=&verkocht=1"%self.catalognumber
    ...
    def parseDetailPage(self, detailsPage, artistname, artwork_name, auction_number, lot_number, imagepath, downloadimages):
        ...
        if descdivtags.__len__() > 0:
            ...
            print(allptags_1)
            try:
                zps1=re.findall("([\d\.\s,\/]+\s*x\s*[\d\.\s,\/]+\s*mm\.?)",allptags_1[0].getText().replace(',',''))
                print(allptags_1[0])
                print("+++++++++++++++++++++")
                print(allptags_1[-1])
                print(zps1)
                detailData['artwork_size_notes'] = zps1[0] 
                detailData['auction_measureunit'] = "mm"
            except:
                pass
            try:
                zps1=re.findall("([\d\.\s,\/]+\s*x\s*[\d\.\s,\/]+\s*mm\.?)",allptags_1[-1].getText().replace(',',''))  
                print(zps1)
                print("+++++++++++++++++++++")
                detailData['artwork_size_notes'] = zps1[0] 
                detailData['auction_measureunit'] = "mm"
                ...
                pass
            try:
                zps1=re.findall("([\d\.\s,\/]+\s*x\s*[\d\.\s,\/]+\s*cm\.?)",allptags_1[0].getText().replace(',',''))
              
                detailData['artwork_size_notes'] = zps1[0] 
                detailData['auction_measureunit'] = "cm"
            except:
                pass
            if "Afmetingen" in allptags_1[-1]:
                a=allptags_1.split("Afmetingen")
                print(a)
                print("---------------------------------------------")
            try:
...
if __name__ == "__main__":
    ...
    while True:
        ...
        requestUrl = "https://www.bernaerts.eu/tags/veiling_ajax.php?cookie=&cataloog=%s&zitting=3&sql_sort_method=0&start_page=0&show_rows=250&sql_lot_range=&lijst=0&search_mode=&search_data=&verkocht=1"%(bernaerts.catalognumber)
        print(requestUrl)
        bernaerts.currentPageContent = bernaerts.getNextPage(requestUrl)
    fp.close()
    updatestatus(auctionnumber, auctionurl)

# Example: python bernaerts.py "https://www.bernaerts.eu/en/auctions/auction-results-grid/?veil=c1088&zitting=3&verkocht=1" c1088  /Users/saiswarupsahu/freelanceprojectchetan/bernaerts_c1088_3.csv  /Users/saiswarupsahu/freelanceprojectchetan/Aguttes/Image/121480   0  0 
