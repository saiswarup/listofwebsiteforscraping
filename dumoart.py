...
class DumoartBot(object):
    ...
    def __init__(self, auctionurl, auctionnumber):
        ...
        self.auctiondate = ""
        self.auctiontitle = ""
        self.auctionlocation = "Detroit, MI"
    ...
    def getLotsFromPage(self):
        ...
        if titletag is not None:
            ...
            self.auctiontitle = self.auctiontitle.replace('"', "'")
            self.auctiontitle = self.auctiontitle.replace(" | DuMouchelles", "")
    ...
    def parseDetailPage(self, detailsPage, artistname, artwork_name, auction_number, lot_number, imagepath, downloadimages):
        ...
        metatag = soup.find("meta", {'property' : 'og:image:secure_url'})
        if metatag is not None:
            imgurl = metatag.attrs['content']
    ...
    def getInfoFromLotsData(self, htmldivtags, imagepath, downloadimages):
        baseUrl = "https://live.dumoart.com"
        info = []
        ...
        sizePattern5 = re.compile("H\s+([\d\.,\/]+)\",?\s+W\s+([\d\.,\/]+)\",?\s+D\s+([\d\.,\/]+)\"", re.DOTALL|re.IGNORECASE)
        sizePattern6 = re.compile("H\s+([\d\.,\/]+)\",?\s+W\s+([\d\.,\/]+)\"", re.DOTALL|re.IGNORECASE)
        ...
        natPattern = re.compile("\(([a-zA-Z\/\s]+),?\s*\-\s*\)", re.DOTALL)
        ...
        for lotinfo in htmldivtags:
            ...
            data['auction_house_name'] = "Dumoart"
            lotno = ""
            detailUrl = ""
            ...
            if not data['artwork_size_notes'] or data['artwork_size_notes'] == "":
                zps1 = re.search(sizePattern1, str(lotinfo['title']))
                zps2 = re.search(sizePattern2, str(lotinfo['title']))
                zps3 = re.search(sizePattern3, str(lotinfo['title']))
                zps4 = re.search(sizePattern4, str(lotinfo['title']))
                zps5 = re.search(sizePattern5, str(lotinfo['title']))
                zps6 = re.search(sizePattern6, str(lotinfo['title']))
                if zps1:
                    data['artwork_size_notes'] = zps1.groups()[0]
                    title = sizePattern1.sub("", title)
                elif zps2:
                    data['artwork_size_notes'] = zps2.groups()[0]
                    title = sizePattern2.sub("", title)
                elif zps3:
                    data['artwork_size_notes'] = zps3.groups()[0]
                    title = sizePattern3.sub("", title)
                elif zps4:
                    data['artwork_size_notes'] = zps4.groups()[0]
                    title = sizePattern4.sub("", title)
                elif zps5:
                    data['artwork_size_notes'] = zps5.groups()[0] + " x " + zps5.groups()[1] + " x " + zps5.groups()[2] + " in"
                    data['artwork_measurements_height'] = zps5.groups()[0]
                    data['artwork_measurements_width'] = zps5.groups()[1]
                    data['artwork_measurements_depth'] = zps5.groups()[2]
                    data['auction_measureunit'] = "in"
                    title = sizePattern5.sub("", title)
                elif zps6:
                    data['artwork_size_notes'] = zps6.groups()[0] + " x " + zps6.groups()[1] + " in" 
                    data['artwork_measurements_height'] = zps6.groups()[0]
                    data['artwork_measurements_width'] = zps6.groups()[1]
                    data['auction_measureunit'] = "in"
                    title = sizePattern6.sub("", title)
            titleparts = re.split(re.compile("[\),]{1}", re.DOTALL), title)
            for tpart in titleparts:
                mps = re.search(mediumPattern, tpart)
                if mps:
                    data['artwork_materials'] = tpart
                    data['artwork_materials'] = data['artwork_materials'].replace('"', "'")
            data['artwork_name'] = titleparts[-1] # Last element of the list
            data['artwork_name'] = data['artwork_name'].replace('"', "'")
            if artistinfo == "" or not artistinfo or artistinfo == "None":
            ...
            if bdps1:
                ...
                data['artist_birth'] = ""
                data['artist_death'] = ""
            nps = re.search(natPattern, data['artist_name'])
            if nps:
                data['artist_nationality'] = nps.groups()[0]
                data['artist_name'] = natPattern.sub("", data['artist_name'])
            ...
            print(data['lot_num'] + " ## " + data['price_estimate_max'] + " ## " + data['artwork_size_notes'] + " ## " + data['price_sold'] + " ## " + data['artwork_name'])
...
def updatestatus(auctionno, auctionurl):
    ...
    pageurl = "http://216.137.189.57:8080/scrapers/finish/?scrapername=Dumoart&auctionnumber=%s&auctionurl=%s&scrapertype=2"%(auctionno, urllib.parse.quote(auctionurl))
    pageResponse = None
    try:
...
if __name__ == "__main__":
    ...
    dumobot = DumoartBot(auctionurl, auctionnumber)
    fp = open(csvpath, "w")
    ...
    lastlotstartnum = 0
    while True:
        soup = BeautifulSoup(dumobot.currentPageContent, features="html.parser")
        lotsdata = dumobot.getLotsFromPage()
        if lotsdata.__len__() == 0:
            break
        info = dumobot.getInfoFromLotsData(lotsdata, imagepath, downloadimages)
        if info.__len__() > 0:
        ...
        dumobot.currentPageContent = dumobot.getNextPage(nextpageurl)
    fp.close()
    updatestatus(auctionnumber, auctionurl)

# Example: python dumoart.py "https://live.dumoart.com/auctions/4-4P6RNO/the-january-auctions-day-2" 4-4P6RNO /home/supmit/work/art2/dumoart_4-4P6RNO.csv /home/supmit/work/art2/images/dumoart/4-4P6RNO 0 0
# https://live.dumoart.com/auctions/4-4P6RNO/the-january-auctions-day-2
